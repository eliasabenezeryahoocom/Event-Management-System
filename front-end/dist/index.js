...
(function (EventApp) {
    ...
    // RSVP click handler
    const onRSVPClick = async (eventId, eventName) => {
    ...
    const addRSVPEventListener = (eventId, eventName) => {
        const rsvpButton = document.getElementById(`buy-ticket-${eventId}`);
        if (rsvpButton) {
    ...
    const addDeclineEventListener = (eventId) => {
        const declineButton = document.getElementById(`decline-ticket-${eventId}`);
        if (declineButton) {
